...
{
    purrr::pwalk(x %>% dplyr::filter(make_s3 == T), ~write_scripts_to_mk_r3_clss(name_stub = ..2, 
    ...
    purrr::pwalk(x %>% dplyr::filter(make_s3 != T), ~write_scripts_to_mk_r4_clss(name_stub = ..2, 
        name_prefix = name_prefix, output_folder = output_dir, 
        class_desc = ..10, parent = if (is.na(..11)) {
            NULL
        }
