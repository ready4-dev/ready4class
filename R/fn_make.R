...
#' @param checker_1L_chr Checker (a character vector of length one), Default: 'NA'
#' @param parent_cls_nm_1L_chr Parent class name (a character vector of length one)
#' @param prototype_lup Prototype (a lookup table)
#' @param prepend_1L_lgl Prepend (a logical vector of length one), Default: T
#' @return Child class function body (a character vector of length one)
    ...
    if (!is.null(parent_cls_nm_1L_chr)) {
        ...
        if (prepend_1L_lgl) 
            new_fn_chr <- paste0(new_fn_chr, "%>% dplyr::select(c(", 
                ...
                "))")
        child_cls_fn_body_1L_chr <- paste0(checker_1L_chr, new_fn_chr)
    }
    else {
...
#' @export 
#' @importFrom stringr str_c
#' @importFrom purrr pmap_chr map_chr map2_chr
#' @keywords internal
    ...
    class_nm_1L_chr, parent_cls_nm_1L_chr, prototype_lup) 
{
    if (type_1L_chr %in% c("tibble", "list")) {
        checker_1L_chr <- paste0("arg_lgths_dbl <- list(", names(vals_ls) %>% 
            stringr::str_c(sep = "", collapse = ",\n"), ") %>% purrr::map_dbl(~length(.x))\n", 
            "arg_max_lgth_1L_dbl <- max(arg_lgths_dbl)\n", "if(arg_max_lgth_1L_dbl >0){\n", 
            purrr::pmap_chr(list(names(vals_ls), 1:length(vals_ls), 
                vals_ls), ~{
                val_xx <- eval(parse(text = ..3))
                paste0("if(0 == arg_lgths_dbl[", ..2, "] & arg_lgths_dbl[", 
                  ..2, "] != arg_max_lgth_1L_dbl){\n", ..1, " <- ", 
                  ifelse(is.character(val_xx), "NA_character_", 
                    ifelse(is.integer(val_xx), "NA_integer_", 
                      ifelse(is.complex(val_xx), "NA_complex_", 
                        ifelse(is.numeric(val_xx), "NA_real_", 
                          ifelse(is.logical(val_xx), "NA", "list(NULL)"))))), 
                  "\n}\n")
            }) %>% stringr::str_c(sep = "", collapse = "\n"), 
            "}\n")
        main_body_1L_chr <- paste0(ifelse(type_1L_chr == "tibble", 
            "tibble::tibble(", "list("), purrr::map_chr(names(vals_ls), 
            ~paste0(.x, " = ", .x)) %>% stringr::str_c(sep = "", 
            collapse = ",\n"), ")")
        fn_call_to_create_prototype <- make_child_cls_fn_body(child_ext_fn_1L_chr = main_body_1L_chr, 
            checker_1L_chr = checker_1L_chr, parent_cls_nm_1L_chr = parent_cls_nm_1L_chr, 
            prototype_lup = prototype_lup, prepend_1L_lgl = T)
    }
    else {
