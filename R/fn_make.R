...
#' @return NULL
#' @rdname make_and_tf_curr_gen_ls
#' @export 
#' @importFrom gtools getDependencies
#' @importFrom purrr discard
#' @importFrom ready4fun unload_packages
#' @keywords internal
make_and_tf_curr_gen_ls <- function (required_pckg_chr_vec, generic_chr, ignore_ns_chr) 
{
...
#' @param prepend_lgl Prepend (a logical vector of length 1), Default: T
#' @return NULL
#' @rdname make_child_fn_chr
#' @export 
#' @importFrom stringr str_c
#' @keywords internal
make_child_fn_chr <- function (child_ext_fn_chr, parent_chr, prototype_lup, prepend_lgl = T) 
{
...
{
    purrr::map2_dfr(class_mk_ls, names(class_mk_ls), ~make_class_type_mk_tb(.x, 
...
#' @keywords internal
make_class_type_mk_tb <- function (class_type_mk_ls, make_s3_lgl = T) 
{
...
#' @return NULL
#' @rdname make_className_chr
#' @export 

#' @keywords internal
make_className_chr <- function (class_chr, package_chr = ".GlobalEnv") 
{
...
#' @param generic_chr Generic (a character vector of length 1)
#' @return NULL
#' @rdname make_current_generics_ls
#' @export 
...
#' @keywords internal
make_current_generics_ls <- function (required_pckg_chr_vec, generic_chr) 
{
    if (!required_pckg_chr_vec %>% purrr::discard(is.na) %>% 
        identical(character(0))) 
...
#' @keywords internal
make_gen_fn_chr <- function (name_chr, args_chr_vec) 
{
...
#' @keywords internal
make_gen_mthd_pair_ls <- function (name_chr, args_chr_vec = c("x"), signature_chr = NA_character_, 
    package_chr = NA_character_, where_chr = NA_character_, class_chr, 
    fn) 
{
    list(generic_chr = make_generic_chr(name_chr, args_chr_vec = args_chr_vec, 
        signature_chr = signature_chr, where_chr = where_chr), 
        method_chr = make_method_chr(name_chr, class_chr = class_chr, 
            fn = fn, package_chr = package_chr, where_chr = where_chr), 
        gen_fn_chr = make_gen_fn_chr(name_chr, args_chr_vec = args_chr_vec), 
...
#' @keywords internal
make_generic_chr <- function (name_chr, args_chr_vec = c("x"), signature_chr = NA_character_, 
...
#' @param ignore_ns_chr Ignore namespace (a character vector of length 1)
#' @return NULL
#' @rdname make_import_packages_ls
#' @export 
#' @importFrom purrr map2_lgl
#' @keywords internal
make_import_packages_ls <- function (current_generics_ls, fn_name_chr, ignore_ns_chr) 
{
...
#' @return NULL
#' @rdname make_include_tag_chr
#' @export 
#' @importFrom stringr str_c
#' @keywords internal
make_include_tag_chr <- function (class_names_chr_vec, s3_lgl = T) 
{
    ifelse(!is.null(class_names_chr_vec), paste0("#' @include ", 
        get_class_files_chr(class_names_chr_vec = class_names_chr_vec, 
...
#' @keywords internal
make_method_chr <- function (name_chr, class_chr, fn, package_chr = NA_character_, 
    where_chr = NA_character_) 
{
    paste0("methods::setMethod(\"", name_chr, "\"", ", ", ifelse(is.na(package_chr[1]), 
        paste0("\"", class_chr, "\""), paste0(make_className_chr(class_chr, 
            package_chr = package_chr))), ", ", make_meth_fn_chr(fn), 
        ifelse(is.na(where_chr[1]), "", paste0(",\nwhere =  ", 
            where_chr)), ")")
}
#' Make new classes
...
#' @importFrom ready4fun write_pt_lup_db_R
#' @importFrom devtools document load_all
#' @keywords internal
make_new_classes <- function (new_classes_ls, pckg_name_chr, class_pfx_chr, R_dir_chr = "R", 
    pt_lup, description_ls = NULL, ignore_ns_chr = NA_character_, 
    required_pckg_chr_vec = NA_character_) 
{
    reset_pkg_files_R(pckg_name_chr, description_ls = description_ls)
    pt_lup <- make_class_mk_tb(new_classes_ls) %>% make_and_update(dev_pckg_namespace = pckg_name_chr, 
        name_prefix = class_pfx_chr, output_dir = R_dir_chr, 
        file_exists_logic = "overwrite", init_class_pt_lup = pt_lup, 
...
#' @param class_desc PARAM_DESCRIPTION, Default: ''
#' @param parent PARAM_DESCRIPTION, Default: NULL
#' @param type PARAM_DESCRIPTION
#' @param type_namespace PARAM_DESCRIPTION, Default: ''
#' @param type_checker_prefix PARAM_DESCRIPTION, Default: 'is.'
#' @param values PARAM_DESCRIPTION, Default: NULL
#' @param ordered PARAM_DESCRIPTION, Default: FALSE
#' @param allowed_values PARAM_DESCRIPTION, Default: NULL
#' @param min_max_values PARAM_DESCRIPTION, Default: NULL
#' @param start_end_values PARAM_DESCRIPTION, Default: NULL
#' @param prototype_lup Prototype (a lookup table)
#' @param ignore_ns_chr Ignore namespace (a character vector of length 1), Default: 'NA'
#' @param file_exists_logic PARAM_DESCRIPTION, Default: 'skip'
#' @return NULL
#' @rdname make_ready_s3
#' @export 
#' @importFrom purrr pwalk
#' @importFrom ready4fun close_open_sinks
#' @importFrom devtools document load_all
#' @keywords internal
make_ready_s3 <- function (name_stub, name_prefix = "ready4_", output_folder = "data-raw", 
    class_desc = "", parent = NULL, type, type_namespace = "", 
    type_checker_prefix = "is.", values = NULL, ordered = FALSE, 
    allowed_values = NULL, min_max_values = NULL, start_end_values = NULL, 
    prototype_lup, ignore_ns_chr = NA_character_, file_exists_logic = "skip") 
{
    if (!dir.exists(output_folder)) 
        dir.create(output_folder)
    class_name <- paste0(name_prefix, name_stub)
    class_file_chr <- get_class_files_chr(class_names_chr_vec = class_name, 
        s3_lgl = T, output_dir_chr = output_folder)
    ...
    if (file_exists_logic %in% c("append", "overwrite")) {
        s3_components_ls <- make_s3_components_ls(class_name_chr = class_name, 
            type_chr = type, type_ns_chr = type_namespace, type_checker_pfx_chr = type_checker_prefix, 
            values_chr = values, ordered_lgl = ordered, parent_chr = parent, 
            prototype_lup = prototype_lup, min_max_values_dbl_vec = min_max_values, 
            start_end_values_dbl_vec = start_end_values, ignore_ns_chr = ignore_ns_chr)
        sink(class_file_chr, append = ifelse(file_exists_logic == 
            "append", TRUE, FALSE))
        writeLines(s3_components_ls$include_tags_chr)
        purrr::pwalk(list(s3_components_ls$fn_name_ls, s3_components_ls$fn_text_ls, 
            c("s3_valid_instance", "s3_unvalidated_instance", 
                "s3_prototype", "s3_validator", "s3_checker")), 
            ~write_fn_txt_and_tags(fn_name = ..1, fn_text = ..2, 
                fn_type = ..3, class_name = class_name, class_desc = class_desc))
...
#' @return NULL
#' @rdname make_ready_s3_checker
#' @export 

#' @keywords internal
make_ready_s3_checker <- function (class_name, s3_validator) 
{
    ...
    fn_to_check_if_is_valid_instance <- paste0(name_of_fn_to_check_if_is_valid_instance, 
        " <- function(x) inherits(", s3_validator$fn_name, "(x), \"", 
        class_name, "\")")
...
#' @return NULL
#' @rdname make_ready_s3_constructor
#' @export 

#' @keywords internal
make_ready_s3_constructor <- function (type, type_checker_prefix, type_namespace, class_name, 
    s3_prototype) 
{
    name_of_fn_to_construct_instance <- paste0("new_", class_name)
    stop_cndn_in_constructor <- ifelse(type == "factor", "TRUE", 
        paste0(type_namespace, ifelse(type_namespace == "", "", 
            "::"), type_checker_prefix, type, "(x)"))
    fn_to_construct_instance <- paste0(name_of_fn_to_construct_instance, 
        " <- function(x){ \n", "stopifnot(", stop_cndn_in_constructor, 
        ")\n", "class(x) <- append(", "c(\"", class_name, "\",setdiff(", 
        paste0(s3_prototype$fn_name, "()"), " %>% class(),class(x)))", 
        ",\nclass(x))\nx\n}")
...
#' @param ordered PARAM_DESCRIPTION
#' @param class_name PARAM_DESCRIPTION
#' @param parent_chr Parent (a character vector of length 1)
#' @param prototype_lup Prototype (a lookup table)
#' @return NULL
#' @rdname make_ready_s3_prototype
#' @export 
#' @importFrom purrr map2_chr
#' @importFrom stringr str_c
#' @keywords internal
make_ready_s3_prototype <- function (type, type_namespace, values, ordered, class_name, 
    parent_chr, prototype_lup) 
{
    if (type %in% c("tibble", "list")) {
        fn_call_to_create_prototype <- paste0(ifelse(type == 
            "tibble", "tibble::tibble(", "list("), purrr::map2_chr(names(values), 
            values, ~paste0(.x, " = ", .y)) %>% stringr::str_c(sep = "", 
            collapse = ",\n"), ")")
        fn_call_to_create_prototype <- make_child_fn_chr(child_ext_fn_chr = fn_call_to_create_prototype, 
            parent_chr = parent_chr, prototype_lup = prototype_lup, 
            prepend_lgl = T)
    }
    else {
        if (type == "factor") {
            fn_call_to_create_prototype <- paste0("factor(x = character(),\nlevels=c(\"", 
                values %>% stringr::str_c(sep = "", collapse = "\",\"\n"), 
                "\"),\nordered=", ordered, ")")
        }
        else {
            fn_call_to_create_prototype <- paste0(type_namespace, 
                ifelse(type_namespace == "", "", "::"), type, 
                "(0)")
        }
    }
    name_of_fn_to_make_prototype <- paste0("make_prototype_", 
        class_name)
...
#' @param s3_validator PARAM_DESCRIPTION
#' @return NULL
#' @rdname make_ready_s3_valid_instance
#' @export 

#' @keywords internal
make_ready_s3_valid_instance <- function (class_name, s3_prototype, s3_constructor, s3_validator) 
{
    fn_call_to_make_valid_instance <- paste0(s3_validator$fn_name, 
        "(", s3_constructor$fn_name, "(x))")
    name_of_fn_to_make_valid_instance <- class_name
    fn_to_make_valid_instance <- paste0(name_of_fn_to_make_valid_instance, 
        " <- function(x = ", s3_prototype$fn_name, "()){ \n", 
...
#' @param class_name PARAM_DESCRIPTION
#' @param s3_prototype PARAM_DESCRIPTION
#' @param min_max_values PARAM_DESCRIPTION
#' @param start_end_values PARAM_DESCRIPTION
#' @param values PARAM_DESCRIPTION
#' @return NULL
#' @rdname make_ready_s3_validator
#' @export 
#' @importFrom purrr compact map2_chr
#' @importFrom stringr str_c
#' @keywords internal
make_ready_s3_validator <- function (type, class_name, s3_prototype, min_max_values, start_end_values, 
    values) 
{
    ...
    if (type %in% c("tibble", "list")) {
        stop_cndn_in_validator_1 <- paste0("sum(stringr::str_detect(names(x)[names(x) %in% names(", 
            s3_prototype$fn_name, "())],\n", "names(", s3_prototype$fn_name, 
            "())))!=length(names(", s3_prototype$fn_name, "()))")
        tb_or_ls_class_summary <- ifelse(type == "list", "lapply(class) %>% tibble::as_tibble() ", 
        ...
        stop_cndn_in_validator_2 <- paste0("!identical(", var_class_lup, 
            ...
            tb_or_ls_class_summary, "%>% \n tidyr::gather(variable,class) %>% \n", 
            "dplyr::filter(variable %in% names(", s3_prototype$fn_name, 
            "())) %>% ", "dplyr::arrange(variable)", ")")
        obj_components_vec <- c(toupper(type), ifelse(type == 
            "list", "elements", "columns"))
        stop_msg_call_in_validator_1 <- paste0("paste0(\"", obj_components_vec[1], 
            " must include ", obj_components_vec[2], " named: \",\n", 
            "names(", s3_prototype$fn_name, "()) %>% stringr::str_c(sep=\"\", collapse = \", \"))")
        stop_msg_call_in_validator_2 <- paste0("paste0(\"", obj_components_vec[1], 
            " ", obj_components_vec[2], " should be of the following classes: \",\n", 
        ...
        if (!is.null(start_end_values)) {
            ...
            if (!is.na(start_end_values[1])) {
                stop_cndn_in_validator_1 <- paste0("any(purrr::map_lgl(x, ~ !startsWith(.x,\"", 
                  start_end_values[1], "\")))")
                stop_msg_call_in_validator_1 <- paste0("\"All values in valid ", 
                  class_name, " object must start with '", start_end_values[1], 
                  "'.\"")
            }
            if (!is.na(start_end_values[2])) {
                stop_cndn_in_validator_2 <- paste0("any(purrr::map_lgl(x, ~ !endsWith(.x,\"", 
                  start_end_values[2], "\")))")
                stop_msg_call_in_validator_2 <- paste0("\"All values in valid ", 
                  class_name, " object must end with '", start_end_values[2], 
                  "'.\"")
            }
        ...
        if (type == "factor") {
            ...
            stop_msg_call_in_validator_1 <- paste0("\"Levels in valid ", 
                class_name, " object are: ", values %>% stringr::str_c(collapse = ","), 
                ".\"")
            validator_stop_cond_ls <- list(a = stop_cndn_in_validator_1)
            validator_stop_msg_call_ls <- list(a = stop_msg_call_in_validator_1)
        }
...
#' @param name_prefix PARAM_DESCRIPTION, Default: 'ready4_'
#' @param output_folder PARAM_DESCRIPTION, Default: 'data-raw'
#' @param output_sub_folder PARAM_DESCRIPTION, Default: NULL
#' @param class_desc PARAM_DESCRIPTION, Default: ''
#' @param parent PARAM_DESCRIPTION, Default: NULL
#' @param class_slots PARAM_DESCRIPTION
#' @param type PARAM_DESCRIPTION
#' @param meaningful_names PARAM_DESCRIPTION, Default: NULL
#' @param values PARAM_DESCRIPTION, Default: NULL
#' @param allowed_values PARAM_DESCRIPTION, Default: NULL
#' @param include_classes PARAM_DESCRIPTION, Default: NULL
#' @param prototype_lup Prototype (a lookup table)
#' @param ignore_ns_chr Ignore namespace (a character vector of length 1), Default: 'NA'
#' @param required_pckg_chr_vec Required pckg (a character vector), Default: 'NA'
#' @param names_include PARAM_DESCRIPTION, Default: NULL
#' @param not_same_length PARAM_DESCRIPTION, Default: NULL
#' @param print_set_class PARAM_DESCRIPTION, Default: TRUE
#' @param print_helper PARAM_DESCRIPTION, Default: TRUE
#' @param print_accessors PARAM_DESCRIPTION, Default: TRUE
#' @param print_validator PARAM_DESCRIPTION, Default: TRUE
#' @param print_meaningful_names PARAM_DESCRIPTION, Default: TRUE
#' @param class_in_cache_logic_chr Class in cache logic (a character vector of length 1), Default: 'stop'
#' @return NULL
#' @rdname make_ready_s4
#' @export 
#' @importFrom ready4fun close_open_sinks
#' @importFrom stringr str_replace str_replace_all
#' @importFrom devtools document load_all
#' @keywords internal
make_ready_s4 <- function (name_stub, name_prefix = "ready4_", output_folder = "data-raw", 
    output_sub_folder = NULL, class_desc = "", parent = NULL, 
    class_slots, type, meaningful_names = NULL, values = NULL, 
    allowed_values = NULL, include_classes = NULL, prototype_lup, 
    ignore_ns_chr = NA_character_, required_pckg_chr_vec = NA_character_, 
    names_include = NULL, not_same_length = NULL, print_set_class = TRUE, 
    print_helper = TRUE, print_accessors = TRUE, print_validator = TRUE, 
    print_meaningful_names = TRUE, class_in_cache_logic_chr = "stop") 
{
    if (!is.null(output_sub_folder)) {
        output_folder <- paste0(output_folder, "/", output_sub_folder)
        if (!dir.exists(output_folder)) 
            dir.create(output_folder)
    }
    proto_ls <- get_proto_list(class_slots = class_slots, type = type, 
        values = values, prototype_lup = prototype_lup)
    class_name <- paste0(name_prefix, name_stub)
    output_file_class <- get_class_files_chr(class_names_chr_vec = class_name, 
        s3_lgl = F, output_dir_chr = output_folder)
    parent_ns_ls <- get_parent_ns_ls(prototype_lup = prototype_lup, 
        parent_chr = parent, dev_pckg_ns_chr = ignore_ns_chr[1])
    set_ready_class(class_name = class_name, class_slots = class_slots, 
        type = type, proto_ls = proto_ls, parent = parent, print_set_class = print_set_class, 
        class_desc = class_desc, output_file_class = output_file_class, 
        include_classes = include_classes, prototype_lup = prototype_lup, 
        helper_lgl = print_helper, parent_ns_ls = parent_ns_ls)
    helper_function <- create_ready_helper(class_name = class_name, 
        parent = parent, class_slots = class_slots, proto_ls = proto_ls, 
        prototype_lup = prototype_lup, parent_ns_ls = parent_ns_ls)
    eval(parse(text = helper_function))
    if (print_helper) {
        sink(output_file_class, append = TRUE)
        write_fn_dmt(fn_name_chr = class_name, fn_type_chr = "set_class", 
            fn = eval(parse(text = class_name)), class_name_chr = class_name)
        writeLines(helper_function)
        ready4fun::close_open_sinks()
    }
    accessors <- create_ready_accessors(class_name = class_name, 
        parent = parent, print_accessors = print_accessors, output_folder = output_folder, 
        ignore_ns_chr = ignore_ns_chr, required_pckg_chr_vec = required_pckg_chr_vec, 
        parent_ns_ls = parent_ns_ls)
    eval(parse(text = accessors %>% replace_NA_in_fn_chr()))
    valid_txt <- validate_ready(class_name = class_name, parent = parent, 
        not_same_length = not_same_length, allowed_values = allowed_values, 
...
#' @param type_checker_pfx_chr Type checker prefix (a character vector of length 1)
#' @param values_chr Values (a character vector of length 1)
#' @param ordered_lgl Ordered (a logical vector of length 1)
#' @param parent_chr Parent (a character vector of length 1)
#' @param prototype_lup Prototype (a lookup table)
#' @param min_max_values_dbl_vec Min max values (a double vector)
#' @param start_end_values_dbl_vec Start end values (a double vector)
#' @param ignore_ns_chr Ignore namespace (a character vector of length 1)
#' @return NULL
#' @rdname make_s3_components_ls
#' @export 

#' @keywords internal
make_s3_components_ls <- function (class_name_chr, type_chr, type_ns_chr, type_checker_pfx_chr, 
    ...
    start_end_values_dbl_vec, ignore_ns_chr) 
{
    ...
    include_tags_chr <- get_parent_ns_ls(prototype_lup = prototype_lup, 
        ...
        get_included_classes_chr_vec(parent_chr = parent_chr, 
            prespecified_includes_chr = NULL) %>% make_include_tag_chr(s3_lgl = T)
...
{
    make_class_type_mk_tb(list(name_stub = x@name_stub_chr, prototype = x@prototype_ls, 
        prototype_checker_prefix = x@prototype_chk_pfx_ls, prototype_namespace = x@prototype_ns_ls, 
        values = x@values_ls, allowed_values = x@allowed_values_ls, 
        min_max_values = x@min_max_values_ls, start_end_values = x@start_end_values_ls, 
