# ready4class

## Author Computational Health Economic Model Modules

<!-- badges: start -->
[![Lifecycle: experimental](https://img.shields.io/badge/lifecycle-experimental-orange.svg)](https://lifecycle.r-lib.org/articles/stages.html#experimental)
[![ready4](https://img.shields.io/badge/ready4-authoring-maroon?style=flat&labelColor=black&logo=data:image/png;base64,)](https://www.ready4-dev.com/toolkits/)
[![R-CMD-check](https://github.com/ready4-dev/ready4class/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/ready4-dev/ready4class/actions/workflows/R-CMD-check.yaml)
[![test-coverage](https://github.com/ready4-dev/ready4class/actions/workflows/test-coverage.yaml/badge.svg)](https://github.com/ready4-dev/ready4class/actions/workflows/test-coverage.yaml)
[![codecov](https://codecov.io/gh/ready4-dev/ready4class/branch/master/graph/badge.svg)](https://codecov.io/gh/ready4-dev/ready4class)
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.5640313.svg)](https://doi.org/10.5281/zenodo.5640313)
<!-- badges: end -->

ready4class provides tools to standardise and streamline the
process for authoring modules for models developed with the ready4
framework (https://www.ready4-dev.com/).  This development version of
the ready4class package has been made available as part of the process
of testing and documenting the package. You should only trial this
software if you feel confident that you understand what it does and
have created a sandpit area in which you can safely undertake testing.
If you have any questions, please contact the authors
(matthew.hamilton1@monash.edu).

To install a development version of this software, run the following commands in your R console:

```r
utils::install.packages("devtools")

devtools::install_github("ready4-dev/ready4class")

```
